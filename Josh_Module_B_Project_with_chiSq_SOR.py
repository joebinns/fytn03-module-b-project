...
oimgMat = np.array(oimgIm, dtype = np.int64) # gets array of image.
...
imgMat = np.array(imgIm, dtype = np.int64) # gets array of image.
...
maskMat = np.array(maskIm, dtype = np.int64) # gets array of mask image.
...
i = 0
for submask in reducedSubmasks:
    reducedSubmask1OutPath = "REDUCEDSUBMASK_"+str(i)+".jpg"
    reducedSubmasks[i] = np.array(reducedSubmasks[i])
    reducedSubmask1Im = Image.fromarray(reducedSubmasks[i])
...
workingImgMat = np.array(workingImgMat, dtype = np.int64)
workingImgMat2 = np.array(workingImgMat2, dtype = np.int64)


omega = 1.2        # omega = 1 corresponds to Jacobi

N =  50 # 1000 #count of loops for Jacobi method
n = 0
for i in range(0, len(reducedSubmasks)):
    submask = reducedSubmasks[i]
    n = 0
    while n < N:   
        for y in range(len(submask)):
            for x in range(len(submask[y])):
                ...
                offX = x+reducedSubmasksOrigins[i][0]
                offY = y+reducedSubmasksOrigins[i][1]
                workingImgMat2[offY][offX] \
                    = (1-omega)* \
                        (workingImgMat[offY][offX]) \
                        + (omega/4)* \
                            (workingImgMat[offY-1][offX] \
                            + workingImgMat[offY+1][offX] \
                            + workingImgMat[offY][offX+1] \
                                + workingImgMat[offY][offX-1])
                        
            
        # workingImgMat2, workingImgMat = workingImgMat, workingImgMat2 #reapplying the Jacobi method onto the primed system
        workingImgMat = deepcopy(workingImgMat2)
        n+=1
...
for i in range(0, len(reducedSubmasks)):
    ...
    for y in range(len(submask)):
        for x in range(len(submask[y])):
            offX = x+reducedSubmasksOrigins[i][0]
            offY = y+reducedSubmasksOrigins[i][1]
            if workingImgMat2[offY][offX] > 254:
                workingImgMat2[offY][offX] = 254
            elif workingImgMat2[offY][offX] < 1:
                workingImgMat2[offY][offX] = 0
...
workingImgMat2 = np.array(workingImgMat2, dtype = np.int8) 
